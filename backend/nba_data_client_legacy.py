"""
NBA 数据客户端 - 遗留版本（LEGACY）
使用 nba_api 库接入 NBA.com 官方 API

⚠️ 警告：此文件为历史遗留版本，当前项目使用 nba_data_client.py 作为主力实现。
保留此文件仅用于参考和向后兼容，不建议在新功能中使用。
"""
...
class NBADataClient:
    ...
    def get_lebron_recent_game(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            game_data = {
                ...
                "date": str(
                    last_game.get("GAME_DATE", datetime.now().strftime("%Y-%m-%d"))
                ),
                "result": wl,
    ...
    def get_lebron_career_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 计算总出场时间（分钟）
            total_minutes = (
                int(regular_season["MIN"].sum())
                if "MIN" in regular_season.columns
                else 59390
            )
...
if __name__ == "__main__":
    ...
    if recent_game:
        print(
            f"对手：{recent_game['opponent']}, 日期：{recent_game['date']}, 结果：{recent_game['result']}"
        )
        print(
            f"数据：{recent_game['points']}分 {recent_game['rebounds']}板 {recent_game['assists']}助"
        )

    print("\n【詹姆斯生涯数据】")
    career = client.get_lebron_career_stats()
    print(
        f"出场：{career['games']}场，得分：{career['points']}分，篮板：{career['rebounds']}个，助攻：{career['assists']}次"
    )
